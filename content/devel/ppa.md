---
title: "Personal Package Archives (PPAs)"
weight: 30
---

The Xubuntu Development and QA teams use several PPAs (personal package
archives) for staging, testing and previewing new packages and package
versions. These PPAs can be found under the [Xubuntu Developers](https://launchpad.net/~xubuntu-dev) page on Launchpad. The
current official PPAs for development and testing use are below.

## Xubuntu QA Staging

<https://launchpad.net/~xubuntu-dev/+archive/ubuntu/staging>

The staging PPA for Xubuntu QA. The packages and package versions in
this PPA can be found in the current development version of Xubuntu and
are available for use with the latest stable version.

## Xubuntu QA Experimental

<https://launchpad.net/~xubuntu-dev/+archive/ubuntu/experimental>

The experimental PPA for Xubuntu QA. The packages and package versions
in this PPA may be found in a later version of Xubuntu. Packages that
have passed verification and will be included in the next stable version
of Xubuntu will be moved to the
[Staging](https://launchpad.net/~xubuntu-dev/+archive/ubuntu/staging)
PPA.

## Xubuntu SRU Staging

<https://launchpad.net/~xubuntu-dev/+archive/ubuntu/sru-staging>

The staging PPA for Xubuntu [stable release updates](https://wiki.ubuntu.com/StableReleaseUpdates) (SRU). The
packages and package versions in this PPA are being prepared for
inclusion in a [supported version](https://xubuntu.org/releases/) of
Xubuntu.

## Xubuntu Extras

<https://launchpad.net/~xubuntu-dev/+archive/ubuntu/extras>

The preview PPA for Xubuntu. The packages in this PPA are considered for
inclusion in the Ubuntu repositories and/or Xubuntu at a later time. The
packages in this PPA have not undergone significant testing and are not
guaranteed to be included in a later release.
